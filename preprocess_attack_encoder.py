...
# PREDICT
# -----------------------------
prediction_prob = model.predict(X)  # shape (1,1) for binary or (1,num_classes) if multi-class

# For binary classification
if prediction_prob[0][0] > 0.5:
    # If you trained a multi-class attack model, replace the above with model.predict returning vector
    # For demonstration, we treat binary attack as generic "attack"
    attack_type = "generic_attack"  # Placeholder, replace if multi-class model
    print(f"⚠️ Attack Detected: {attack_type}")
    print("✅ Attack Prevented / Cured")
else:
